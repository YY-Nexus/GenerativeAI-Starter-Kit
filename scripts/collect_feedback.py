...
class FeedbackCollector:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for pr in prs:
                    ...
                    if created_at >= datetime.now(created_at.tzinfo) - timedelta(days=days):
    ...
    def analyze_issue_content(self, issues):
        ...
        keywords = {
            ...
            'docs': ['documentation', 'readme', 'example', 'tutorial', '文档', '示例', '教程'],
            'performance': ['slow', 'fast', 'performance', 'speed', '性能', '速度', '慢'],
            'installation': ['install', 'setup', 'dependency', 'requirements', '安装', '依赖']
        }
    ...
    def analyze_feedback(self, days=30):
        ...
        if issues:
            for issue in issues[:5]:
                ...
                print(f"      👤 {issue['user']['login']} | 📅 {issue['created_at'][:10]} | 💬 {issue.get('comments', 0)}")
        else:
            print("   暂无最近的 Issues")
