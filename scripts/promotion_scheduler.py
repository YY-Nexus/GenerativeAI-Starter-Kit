...
class PromotionScheduler:
    def __init__(self):
        self.platforms = {
            'reddit': {
                'days': ['周一', '周四'],
                'subreddits': ['r/MachineLearning', 'r/Python', 'r/ArtificialIntelligence'],
            ...
            'twitter': {
                'days': ['每日'],
                'content_types': ['技术小贴士', '更新公告', '用户案例']
            },
            'github': {
                'days': ['周日'],
                'activities': ['README更新', '文档完善', 'Issues回复']
            }
        }
    ...
    def generate_weekly_report(self):
        ...
        print(f"📊 推广周报 ({week_start.strftime('%Y-%m-%d')} - {week_end.strftime('%Y-%m-%d')})")
        print("=" * 60)
        ...
        for i in range(7):
            ...
            if activities:
                print(f"  {day.strftime('%m-%d')} ({weekday_zh}): {', '.join(activities)}")
        ...
        print(f"  时间: {next_week_start.strftime('%Y-%m-%d')} - {next_week_end.strftime('%Y-%m-%d')}")
